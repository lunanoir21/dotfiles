    ...
    if [ -f "$finalArt" ] && [ -s "$finalArt" ]; then
        ...
        if [ ! -f "$lockFile" ] && [ -n "$rawUrl" ]; then
            ...
            (
                ...
                isPlaceholder=$(convert "$tempArt"[1x1+0+0] png:- 2>/dev/null | identify -format "%[hex:u.p{0,0}]" - 2>/dev/null | cut -c1-6)
                ...
                if [[ "$isPlaceholder" == "313244" ]] || [[ -z "$isPlaceholder" ]]; then
                    ...
                    colors=$(convert "$tempArt" -resize 50x50! -alpha off +dither -quantize RGB -colors 3 -depth 8 -scale 3x1! png:- 2>/dev/null | identify -format "%[hex:u.p{0,0}] %[hex:u.p{1,0}] %[hex:u.p{2,0}]" - 2>/dev/null)
                    ...
                    opp_raw=$(convert xc:"$c1" -alpha off -negate -depth 8 txt:- 2>/dev/null | grep -E -o '[0-9A-Fa-f]{6}' | head -n 1)
                    if [ -n "$opp_raw" ]; then
