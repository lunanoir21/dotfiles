...
Item {
    ...
    // Dynamic Island power policy is owned by the main settings surface. The
    // island widget only watches these values; it must not create a second
    // settings authority in its own submodule file.
    property string deviceKind: "unset"
    property bool powerPromptEnabled: true
    property string powerSaverMode: "normal"
    property string powerSaverAnimStyle: "tint"
    readonly property var deviceKinds: ["unset", "laptop", "desktop"]
    readonly property var powerSaverModes: ["normal", "battery-saver"]
    readonly property var powerSaverAnimStyles: ["tint", "label", "sheen"]
    ...
    // =========================================================================
    // Hyprland Görünüm / Davranış  (settings.json -> "hypr")
    //
    // settings_watcher.sh bu nesneyi templates/settings.conf.template'e basıyor.
    // Buradaki varsayılanlar watcher'daki jq_hypr varsayılanlarıyla birebir aynı
    // olmak zorunda: ikisi ayrışırsa UI, diskteki conf'tan farklı bir değer
    // gösterir ve kullanıcı hiç dokunmadığı bir ayarı "değişmiş" sanır.
    // =========================================================================
    property var hyprSettings: ({})

    property int  hyprBorderSize: 2
    property int  hyprGapsIn: 4
    property int  hyprGapsOut: 4
    property int  hyprFloatGaps: 6
    property bool hyprResizeOnBorder: true

    property int  hyprRounding: 4
    property real hyprActiveOpacity: 1.0
    property real hyprInactiveOpacity: 1.0
    property bool hyprBlurEnabled: true
    property int  hyprBlurSize: 8
    property int  hyprBlurPasses: 2
    property bool hyprShadowEnabled: false

    property real hyprSensitivity: 0.0
    property bool hyprNaturalScroll: true

    property string hyprFontFamily: "JetBrains Mono"
    property bool   hyprAnimationsEnabled: true
    property int    hyprAnimSpeed: 5

    // settings.json anahtarı -> yukarıdaki property adı.
    // Okuma ve yazma aynı haritayı kullanıyor, böylece yeni bir ayar eklemek
    // tek satır ve iki yön asla birbirinden kaymıyor.
    readonly property var hyprKeyMap: ({
        "borderSize":        "hyprBorderSize",
        "gapsIn":            "hyprGapsIn",
        "gapsOut":           "hyprGapsOut",
        "floatGaps":         "hyprFloatGaps",
        ...
        "blurPasses":        "hyprBlurPasses",
        "shadowEnabled":     "hyprShadowEnabled",
        "sensitivity":       "hyprSensitivity",
        "naturalScroll":     "hyprNaturalScroll",
        "fontFamily":        "hyprFontFamily",
        "animationsEnabled": "hyprAnimationsEnabled",
        "animSpeed":         "hyprAnimSpeed"
    })

    // Anlık önizleme için hyprctl keyword yolları. animSpeed burada yok:
    // tek bir keyword'e karşılık gelmiyor, dokuz ayrı animation satırına
    // yayıldığı için sadece yeniden derlemeyle uygulanabiliyor.
    readonly property var hyprLiveMap: ({
        "borderSize":        "general:border_size",
        "gapsIn":            "general:gaps_in",
        "gapsOut":           "general:gaps_out",
        "floatGaps":         "general:float_gaps",
        ...
        "fontFamily":        "misc:font_family",
        "animationsEnabled": "animations:enabled"
    })

    function loadHyprSettings(h) {
        if (!h) return;
        config.hyprSettings = h;
        for (let key in config.hyprKeyMap) {
            if (h[key] !== undefined && h[key] !== null)
                config[config.hyprKeyMap[key]] = h[key];
        }
    }

    // Diske yazmadan, çalışan Hyprland'e tek bir değeri uygular. Slider
    // sürüklerken kullanılacak yol bu: settings.json'a yazmak watcher'ı ve
    // ardından tam bir `hyprctl reload`'u tetiklediği için sürükleme boyunca
    // saniyede onlarca reload demek olurdu.
    function previewHyprSetting(key, value) {
        let kw = config.hyprLiveMap[key];
        if (!kw) return;
        Quickshell.execDetached(["hyprctl", "keyword", kw, String(value)]);
    }

    function saveHyprSettings() {
        let obj = {};
        for (let key in config.hyprKeyMap) obj[key] = config[config.hyprKeyMap[key]];
        config.hyprSettings = obj;
        config.rawSettings["hypr"] = obj;

        let jsonStr = JSON.stringify(obj).replace(/'/g, "'\\''");
        let cmd = `mkdir -p "$(dirname '${settingsJsonPath}')" && ` +
                  `[ ! -f '${settingsJsonPath}' ] && echo '{}' > '${settingsJsonPath}'; ` +
                  `jq '.hypr = (.hypr // {}) + ${jsonStr}' '${settingsJsonPath}' > '${settingsJsonPath}.tmp' && ` +
                  `mv '${settingsJsonPath}.tmp' '${settingsJsonPath}'`;
        sh(cmd);
    }

    // settings.json'a her yazım = template yeniden derlemesi + `hyprctl reload`.
    // UI bu yüzden doğrudan saveHyprSettings() değil bunu çağırmalı.
    Timer {
        id: _hyprSaveDebounce
        interval: 450
        repeat: false
        onTriggered: config.saveHyprSettings()
    }
    function queueHyprSave() { _hyprSaveDebounce.restart(); }
    ...
    function saveAppSettings() {
        let configObj = {
            ...
            "powerPromptEnabled": config.powerPromptEnabled,
            "powerSaverMode": config.powerSaverMode,
            "powerSaverAnimStyle": config.powerSaverAnimStyle
        };
    ...
    function saveDynamicIslandPowerSettings() {
        config.updateJsonBulk({
            "deviceKind": config.deviceKind,
            "powerPromptEnabled": config.powerPromptEnabled,
            "powerSaverMode": config.powerSaverMode,
            "powerSaverAnimStyle": config.powerSaverAnimStyle
            ...
            onStreamFinished: {
                try {
                    if (this.text && this.text.trim().length > 0 && this.text.trim() !== "{}") {
                        ...
                        if (config.deviceKinds.indexOf(config.rawSettings.deviceKind) >= 0)
                            config.deviceKind = config.rawSettings.deviceKind;
                        if (typeof config.rawSettings.powerPromptEnabled === "boolean")
                            config.powerPromptEnabled = config.rawSettings.powerPromptEnabled;
                        if (config.powerSaverModes.indexOf(config.rawSettings.powerSaverMode) >= 0)
                            config.powerSaverMode = config.rawSettings.powerSaverMode;
                        if (config.powerSaverAnimStyles.indexOf(config.rawSettings.powerSaverAnimStyle) >= 0)
                            config.powerSaverAnimStyle = config.rawSettings.powerSaverAnimStyle;
                        ...
                        // Map Hyprland appearance/behaviour block
                        config.loadHyprSettings(config.rawSettings.hypr);
