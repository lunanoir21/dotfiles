...
Item {
    ...
    // Olay güdümlü, sıfır process (bkz. core/Settings.qml'deki aynı desen).
    // Önceden her instance saniyede bir `cat` process'i açan bir Timer
    // taşıyordu; 16 çağrı noktası × 1 fork/sn idle'da gereksiz CPU'ydu.
    // Dosya matugen kaldırıldığından beri zaten sabit — FileView aynı sonucu
    // (dosya değişince güncellenme) process açmadan veriyor.
    function applyJson() {
        let txt = "";
        try { txt = fileView.text().trim(); } catch (e) { return; }
