...
# -----------------------------------------------------------------------------
# ZOMBIE WATCHDOG
# Only runs on slow path — not on every workspace switch. Locked: two
# keybinds fired back to back both saw no process (a pgrep race right after
# exec-once starts it) and each launched their own quickshell, so two full
# shells — two bars, two Quay rails — ran side by side until one was killed
# by hand.
# -----------------------------------------------------------------------------
...
) 9>"$QS_RUN_DIR/shell-watchdog.lock"
