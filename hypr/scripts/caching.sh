...
qs_ensure_cache() {
    local WIDGET_NAME="$1"
    # LC_ALL=C: tr's case folding is locale-dependent - under Turkish locale,
    # lowercase "i" case-folds to "İ" instead of ASCII "I" (e.g.
    # "wallpaper_picker" -> "WALLPAPER_PiCKER", not "WALLPAPER_PICKER"), so
    # QS_CACHE_WALLPAPER_PICKER never actually got set and scripts reading it
    # silently got an empty string. Also map '-' to '_' since bash identifiers
    # can't contain '-' (affects widget folders like "dynamic-island").
    local WIDGET_UPPER=$(echo "$WIDGET_NAME" | LC_ALL=C tr '[:lower:]-' '[:upper:]_')
